...
**Canlı arama (diş hekimi listesi):** Arama kutusuna yazdıkça, sayfa **yenilenmeden** yalnızca sonuç tablosu sunucudan çekilip yerine konur (`?partial=1` → `parties/_results.html`); arama tüm veritabanında yapılır, sadece görünen sayfa filtrelenmez. Odak, imleç ve kaydırma konumu korunur; adres çubuğu `history.replaceState` ile senkron tutulur, böylece yenileme/yer imi çalışır. JavaScript kapalıysa form normal şekilde gönderilir (Enter / "Ara"). İşlem kataloğu araması normal form gönderimi kullanır.
