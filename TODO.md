...
| Test ve kalite güvence | %10 | 92 | 102 test, %75,02 branch coverage ve CI kapısı; gerçek tarayıcı E2E henüz yok |
...
- [x] GitHub Actions üzerinde Python 3.13/3.14, compile, test ve %70 branch coverage kapısı
...
```text
102 passed
Branch coverage: %75,02
Coverage alt sınırı: %70
...
- [ ] Migration gerçek veri kopyasında geçti; geri dönüş planı hazır
- [ ] Kritik E2E ve erişilebilirlik kontrolleri geçti
...
```bash
pytest
pytest --cov=app --cov-branch --cov-report=term-missing --cov-fail-under=70
python -m compileall -q app tests backup.py run.py run_production.py
python -m pip check
git diff --check
```
